 ...
 * @param  {string} bio   the team member's personal profile
 * @return {object} { user, fname, lname, year }
 */
function member(user, fname, lname, year, bio) {
	return {
		user: user,
		fname: fname,
		lname: lname,
...
var team = [
	...
	member('jfiaschetti','Joseph','Fiaschetti,','junior', 'BS in Computer Science with a concentration in Software Engineering. Working on back-end development for InterFridge.'),
	member('awbrown','Aaron','Brown','senior', ''),
	member('cjkerr','Cole','Kerr','senior', ''),
	member('jdparker','Jonathan','Parker','senior', 'Software Engineering concentration at UMass Amherst, interested in Artificial Intelligence, mobile app design and web development. Front-end'),
	member('lyeboah','Linda','Yeboah','senior', 'BA in general computer science, interested in web development.'),
	member('prrapo','Pandi','Rrapo','senior', '')
];
...
function copy(tocopy) {
	return member(tocopy.user, tocopy.fname, tocopy.lname, tocopy.year, tocopy.bio);
}
...
function one(user) {
	...
	if(typeof user === "string"){
		...
		return result(false,"not a string",new Array());
	}
	if(uobject!=null){
		return result(true, "team member found",new Array(uobject));
	}
	return result(false,"team member not found",new Array());
}
